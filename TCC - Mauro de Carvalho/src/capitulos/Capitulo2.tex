...
Pelo fato do \textit{chatbot} também se tratar de um produto digital, algumas práticas e estudos também devem ser realizados em seu processo de criação. Algumas dessas pŕaticas envolvem o levantamento e especificação dos requisitos, a elaboração dos diálogos do \textit{chatbot} e também as tecnologias que serão utilizadas em seu desenvolvimento.
...
Eles podem estar relacionados às propriedades emergentes do sistema, como confiabilidade, tempo de resposta e ocupação de área.\label{texto:requisito_nao_funcional}
...
No caso de usuários, um ator representa na verdade uma função de usuários. Para isso, normalmente se utiliza da notação UML como linguagem de especificação para representar os casos de uso. Abaixo, um exemplo de especificação de casos de uso utilizando UML.

\begin{figure}
\centering
\includegraphics[width=0.9\linewidth]{src/imagens/casosd2.png}
\caption{Caso de uso - Sistema de Vendas. Fonte:~\cite{ReqJair2}}
\label{cap:01:fig:fluxograma}
\end{figure}

\subsection{Elaboração dos diálogos de um \textit{chatbot} a partir de cenários}

Para a elaboração dos diálogos que irão fazer parte do repertório de um \textit{chatbot}, uma técnica utilizada para a sua modelagem e especificação é utilizando os cenários.
...
Cenários são uma maneira de representar, no contexto de \textit{chatbots}, os diálogos e interações e as possibilidades que podem surgir a partir destes. Abaixo, um exemplo de cenário que simula o diálogo entre uma atendente e um consumidor: