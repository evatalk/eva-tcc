...
No que diz respeito a arquitetura de EVA, pode-se dizer que esta consiste em duas partes principais: a API e o \textit{chatbot} propriamente dito. Nas subseções abaixo, serão detalhados as tecnologias que irão compor todo o sistema de EVA e também detalhes sobre a sua implementação.
...
A API de EVA será desenvolvida utilizando o framework Django e terá duas principais funções no sistema como um todo. A primeira, será armazenar a base de dados dos alunos vinculados ao EV.G, para que se possa consultar as informações escolares dos alunos que estiverem se comunicando com o \textit{chatbot}. A segunda, é que a API estará responsável também, por estabelecer uma interoperabilidade com o serviço de PNL fornecido pelo Wit.ai, para que possa compreender a mensagem textual enviada pelo usuário e atende-lo da maneira devida. Para melhor exemplificar como funcionará a API de EVA, na Figura \ref{cap:03:fig:diagrama-sequencia-1} está apresentado o diagrama de sequência da mesma.
\begin{figure}
    \centering
...
Nesta subseção, será explicado como funcionará a o \textit{chatbot} de EVA.
...
Para facilitar, em termos de desenvolvimento, a criação do \textit{chatbot} e integração com a Telegram Bot API, foi utilizado a biblioteca Python-telegram-bot. Esta biblioteca oferece uma interface, em Python, de fácil acesso aos recursos oferecidos pelo Telegram Bot API. FALAR SOBRE A FUNÇÂO GETUPDATE E COMO ELA FUNCIONA.

É válido enfatizar que este \textit{chatbot} de EVA é apenas uma das partes do sistema como um todo. Basicamente, a função deste é enviar a mensagem textual do aluno para a API de EVA e retornar ao aluno a resposta, com base no que foi compreendido, da API de EVA. Para melhor detalhar o fluxo de interações, na Figura \ref{cap:03:fig:diagrama-sequencia-2} é apresentado o diagrama de sequência do \textit{chatbot} de EVA.
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{src/imagens/EVACHATBOTSEQUENCE.png}
    \caption{Diagrama de sequência do \textit{chatbot} de EVA. Fonte: Elaborado pelo autor (2018)}
